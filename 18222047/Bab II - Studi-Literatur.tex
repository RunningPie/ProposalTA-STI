...
\begin{enumerate}
    ...
    (\textit{assessor}) dan yang dinilai (\textit{assessed}) memiliki status
    yang sama" \autocite{Ihichr2024}.
    Evaluasi terhadap kinerja asesor dalam \textit{peer assessment} tidak cukup
    hanya didasarkan pada akurasi skor (\textit{rating accuracy}), tetapi juga
    harus meninjau kualitas umpan balik tekstual yang diberikan. \textcite{Wu2020}
    mengusulkan kerangka kerja yang membedah fitur umpan balik ke dalam dua
    dimensi utama: dimensi kognitif dan dimensi afektif. Dimensi kognitif
    (\textit{cognitive feedback}) berkaitan dengan substansi perbaikan tugas,
    yang terdiri atas empat kategori spesifik: (1) \textit{Identification}, yaitu
    kemampuan asesor mendeteksi letak kesalahan; (2) \textit{Explanation}, yaitu
    penyediaan alasan logis mengapa suatu bagian dianggap salah; (3)
    \textit{Suggestion}, yaitu pemberian saran eksplisit untuk perbaikan; dan (4)
    \textit{Solution}, yaitu penyediaan contoh koreksi langsung.

    Di sisi lain, dimensi afektif (\textit{affective feedback}) mencakup nada
    emosional dan interpresonal yang digunakan asesor, yang terbagi menjadi
    kategori \textit{Praise} (pujian yang memvalidasi kelebihan) dan
    \textit{Hedges} (penggunaan bahasa penyangga untuk memperhalus kritik).
    Dalam sistem yang berorientasi pada pembelajaran kolaboratif, keberadaan
    fitur kognitif tingkat tinggi seperti \textit{Explanation} dan
    \textit{Suggestion} sering kali dijadikan indikator utama kompetensi asesor,
    karena elemen-elemen inilah yang terbukti memicu perbaikan revisi yang
    signifikan pada penerima umpan balik, sekaligus merefleksikan kedalaman
    pemahaman konseptual dari pemberi umpan balik \autocite{Wu2020}.

    Manfaat
    pedagogis dari penilaian sejawat sering diasumsikan berasal dari umpan balik yang diterima siswa.
...
\subsection{Evaluasi Efektivitas Pembelajaran dengan \textit{Normalized LearningGain}}
...
\textit{floor effect} atau \textit{ceiling effect} ekstrem, di mana siswa dengan
